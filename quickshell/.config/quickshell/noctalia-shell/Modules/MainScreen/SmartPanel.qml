  ...
  function setPosition() {
    ...
    if (root.useButtonPosition && root.height > 0 && panelHeight > 0) {
      if (root.barPosition === "top") {
        var topBarEdge = root.barMarginV + Style.barHeight + 135;
        if (panelContent.allowAttach) {
        ...
        var bottomBarEdge = root.height - root.barMarginV - Style.barHeight - 135;
        if (panelContent.allowAttach) {
      ...
      if (!panelContent.allowAttach) {
        if (root.barPosition === "top") {
          barOffset = root.barMarginV + Style.barHeight + Style.marginM + 135;
        } else if (root.barPosition === "bottom") {
          barOffset = root.barMarginV + Style.barHeight + Style.marginM + 135;
        }
      } else {
        if (root.effectivePanelAnchorTop && root.barPosition === "top") {
          calculatedY = root.barMarginV + Style.barHeight + 135;
